...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        GameObject position = GameManager.Instance.NearestPlayer(mRigidBody.transform.position);

        if (position != null) {
            var dir = position.transform.position - mRigidBody.transform.position;
            mRigidBody.rotation = Quaternion.LookRotation(dir, new Vector3(0, 0, 1));
            mRigidBody.MovePosition(mRigidBody.position + Vector3.Normalize(dir)*speed*Time.deltaTime);
        }
    }
