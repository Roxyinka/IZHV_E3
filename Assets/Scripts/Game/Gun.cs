...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if(shotgun){
            float angle = director.eulerAngles.z - shotgunSpread/2;
            for (int i = 0; i < shotgunBullets; i++) {
                angle+=(shotgunSpread/(shotgunBullets+1));
                SpawnBullet(
            ...
            SpawnBullet(
                new Vector3{ x = director.position.x, y = director.position.y, z = director.position.z }, 
                Quaternion.Euler(director.eulerAngles.x+20, director.eulerAngles.y+20, director.eulerAngles.z+20)
            );
            SpawnBullet(
                new Vector3{ x = director.position.x, y = director.position.y, z = director.position.z }, 
                Quaternion.Euler(director.eulerAngles.x-20, director.eulerAngles.y-20, director.eulerAngles.z-20)
